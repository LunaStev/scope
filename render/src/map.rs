    ...
    pub fn paint_map(&mut self,cx:&mut Cx2d,session:&mut Session,view:Box2)->RenderStats {
        ...
        if self.maps.generation!=snapshot.generation {
            match cx.get_dependency(self.map_font.as_str()) {
                Ok(font)=>{let font=Arc::new(font.as_ref().clone());self.atlas.start(font.clone());self.maps.start(snapshot.clone(),font,session.options.max_file_bytes,SignalToUI::set_ui_signal);self.images.clear();self.images.generation=snapshot.generation;self.cache.clear();}
                Err(error)=>{self.text(cx,view.x+24.0,view.y+32.0,12.0,p::unknown(),&shorten(&error,100));return stats;}
            }
        }
        self.maps.poll();self.atlas.poll();let camera=session.camera;let world=snapshot.rectangles[0];let dpi=cx.current_dpi_factor();
        ...
        else if self.atlas.upload(cx){stats.atlas_uploads=1;}
        ...
        if !self.images.ready() {
            ...
            self.text(cx,view.x+24.0,view.y+66.0,11.0,p::secondary(),&shorten(&message,((view.w-48.0)/7.0).max(1.0) as usize));stats.needs_redraw=self.maps.has_images()||self.atlas.waiting_upload();return stats;
        }
        ...
        stats.needs_redraw|=self.atlas.waiting_upload();stats.source_lines=self.maps.prepared_lines as usize;
        ...
        if std::env::var_os("SCOPE_TRACE").is_some(){eprintln!("scope frame: nodes={} lines={} runs={} scale={:.6} font_min={:.4} font_max={:.4} reused={} built={} pending={} waiting=0 limited=0 glyphs={} cpu_ms={:.3} map_ready=1 image_tiles={} uploads={} map_pending={} map_errors={} map_cache_hit={} map_prepare_ms={} redraw={} representation=hybrid-sdf native_glyphs={} native_resident={} native_reused={} native_pending={} map_replans={} cancelled_details={} native_queries={} atlas_ready={} atlas_instances={} atlas_uploads={} atlas_builds={} fallback_builds={} build_cpu_ms={:.4}",stats.visible_nodes,stats.source_lines,stats.glyph_runs,camera.scale,stats.min_font,stats.max_font,stats.reused_chunks,stats.built_chunks,stats.image_pending,stats.built_glyphs,stats.cpu_submit_ms,stats.image_tiles,stats.texture_uploads,stats.image_pending,stats.map_errors,usize::from(stats.map_cache_hit),self.maps.prepare_ms,usize::from(stats.needs_redraw),stats.live_drawn,stats.live_glyphs,stats.live_reused,stats.live_pending,self.maps.replans,self.maps.cancelled_details,self.live.queries,usize::from(self.atlas.resident.is_some()),stats.atlas_instances,stats.atlas_uploads,stats.atlas_builds,stats.fallback_builds,stats.build_cpu_ms);}
        stats
    }
}
