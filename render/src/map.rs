...
impl MapPainter {
    pub fn paint_map(&mut self,cx:&mut Cx2d,session:&mut Session,view:Box2)->RenderStats {
        ...
        self.maps.poll();
        let camera=session.camera;let world=snapshot.rectangles[0];let dpi=cx.current_dpi_factor();
        let desired=if session.sources{raster::tile::requests(world,camera,view,dpi)}else{Vec::new()};
        let protected:HashSet<_>=desired.iter().copied().collect();
        // One bounded texture transfer per paint. Warm camera movement performs
        // no source reads, shaping or glyph uploads on the UI thread.
        if let Some(update)=self.maps.take_image(){self.images.upload(cx,update,world,&protected);stats.texture_uploads=1;}
        let (done,total)=self.maps.progress();stats.map_completed=done;stats.map_total=total;
        stats.map_ready=self.images.ready();stats.map_cache_hit=self.maps.cache_hit;stats.map_errors=self.maps.errors;
        if !self.images.ready() {
            ...
            self.text(cx,view.x+24.0,view.y+66.0,11.0,p::secondary(),&shorten(&message,((view.w-48.0)/7.0).max(1.0) as usize));
            stats.needs_redraw=self.maps.has_images();return stats;
        }
        if self.images.source_mode!=session.sources {self.images.source_mode=session.sources;self.cache.scene=Presentation::default();}
        if session.sources {
            let resident=self.images.resident();self.maps.request(&desired,&resident);
            let (count,fading)=self.images.draw(cx,&mut self.image,camera,view,1.0);
            stats.image_tiles=count;stats.reused_chunks=count;stats.source_lines=self.maps.prepared_lines as usize;
            stats.needs_redraw=fading||self.maps.has_images();
        }
        ...
        if session.sources {scene.discover_labels(&snapshot,camera,view,dpi,&mut stats);}
        else {scene.replay_backgrounds(cx,&mut stats);let _=self.advance_nodes(cx,&mut scene,&snapshot,camera,view,&session.query,&session.matches,&mut stats);}
        ...
        stats.visible_nodes=scene.visible.len();stats.needs_redraw|=scene.unfinished();
        self.cache.scene=scene;
        ...
        if let Some(page)=sample{stats.min_font=page.font_size*camera.scale;stats.max_font=stats.min_font;}
        stats.cpu_submit_ms=started.elapsed().as_secs_f64()*1000.0;
        if std::env::var_os("SCOPE_TRACE").is_some(){
            eprintln!("scope frame: nodes={} lines={} runs=0 scale={:.6} font_min={:.4} font_max={:.4} reused={} built=0 pending={} waiting=0 limited=0 glyphs=0 cpu_ms={:.3} map_ready=1 image_tiles={} uploads={} map_pending={} map_errors={} map_cache_hit={} map_prepare_ms={} redraw={} representation=source-image",stats.visible_nodes,stats.source_lines,camera.scale,stats.min_font,stats.max_font,stats.reused_chunks,stats.image_pending,stats.cpu_submit_ms,stats.image_tiles,stats.texture_uploads,stats.image_pending,stats.map_errors,usize::from(stats.map_cache_hit),self.maps.prepare_ms,usize::from(stats.needs_redraw));
        }
        stats
    }
}
