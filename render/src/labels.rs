//! Header space and clipping use the allocator's world geometry, never an
//! independent screen-space approximation. Numeric values come from the model.
use crate::{MapPainter,palette as p};
use makepad_widgets::*;
use ::layout::{Box2,Camera,node_frame};
use ::model::{Node,format};
impl MapPainter{
    pub(crate) fn node_label(&mut self,cx:&mut Cx2d,node:&Node,bounds:Box2,camera:Camera,view:Box2){
        let header=camera.project(node_frame(bounds).header);let Some(visible)=header.intersection(view)else{return;};
        let size=(header.h/1.6).min(10.0);if size<6.0||visible.w<20.0||visible.h<size*1.25{return;}
        let x=header.x.max(view.x)+3.0;let y=header.y+((header.h-size*1.4)*0.5).max(0.0);
        if y<visible.y-0.5||y+size*1.3>visible.y+visible.h{return;}
        ...
        if show_count{let mut ink=p::muted();ink.w=alpha;self.text_right(cx,right,y,size as f32,ink,&metric);}cx.end_turtle();
    }
    /// Clip edges before f32 conversion. Clipping a large box must not create
    /// a new border at the viewport edge or cover its children with rounding.
    pub(crate) fn box_clipped(&mut self,cx:&mut Cx2d,b:Box2,view:Box2,fill:Vec4,border:Vec4,edge:f64){
        if let Some(r)=b.intersection(view){self.fill(cx,r,fill);}
        for r in [Box2::new(b.x,b.y,b.w,edge),Box2::new(b.x,b.y+b.h-edge,b.w,edge),Box2::new(b.x,b.y,edge,b.h),Box2::new(b.x+b.w-edge,b.y,edge,b.h)]{if let Some(r)=r.intersection(view){self.fill(cx,r,border);}}
    }
}
