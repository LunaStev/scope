...
pub struct MapPainter{
    #[live] pub(crate) quad:DrawColor,
    #[live] pub(crate) label:DrawText,
    #[live] pub(crate) code:DrawText,
}
...
pub struct RenderStats{
    ...
    pub capped:bool,
    /// Wall time on the CPU for submitting this draw; NOT GPU frame time or FPS.
    pub cpu_submit_ms:f64,
}
impl MapPainter{
    pub fn fill(&mut self,cx:&mut Cx2d,b:Box2,color:Vec4){
        if b.w<=0.0||b.h<=0.0{return;}
        self.quad.color=color;self.quad.draw_abs(cx,rect(b));
    }
    pub fn text(&mut self,cx:&mut Cx2d,x:f64,y:f64,size:f32,color:Vec4,text:&str){
        self.label.color=color;self.label.text_style.font_size=size;self.label.draw_abs(cx,dvec2(x,y),text);
    }
}
