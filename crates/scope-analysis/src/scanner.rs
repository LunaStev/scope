use crate::{metrics, options::{ScanOptions, DEFAULT_EXCLUDES}, preview, source::read_text};
use scope_core::{FileInfo, Node, Tree};
...
pub fn scan(root: &Path, options: &ScanOptions, cancel: &AtomicBool) -> Result<Tree, String> {
    ...
    for entry in builder.build() {
        ...
        tree.nodes.push(Node {
            ...
            file: Some(FileInfo { language: measured.language, classified: measured.classified, preview: preview::sample(&text, measured.stats.lines as usize) }),
        });
