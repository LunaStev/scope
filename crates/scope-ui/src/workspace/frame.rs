...
impl Workspace{
    pub(super) fn frame(&mut self,cx:&mut Cx2d,walk:Walk)->DrawStep{
        ...
        let summary=Box2::new(bounds.x,bounds.y,bounds.w,110.0);
        self.dashboard(cx,summary);
        ...
        let view=Box2::new(bounds.x,head.y+head.h,(bounds.w-side).max(1.0),(bounds.h-148.0-32.0).max(1.0));
        ...
        if let Some(id)=self.state.session.focus_pending.take(){
            let world=self.state.session.snapshot.as_ref().and_then(|s|s.rectangles.get(id).copied()).unwrap_or(WORLD);
            self.state.session.camera.fit(world,view);
        }
        clip(cx,view);self.state.frame=self.map.paint_map(cx,&mut self.state.session,view);cx.end_turtle();
        ...
        let status=if self.state.session.busy(){self.state.session.status.clone()}else if self.state.session.snapshot.is_none(){self.state.session.status.clone()}else{"Wheel: zoom  ·  Drag: pan  ·  Double-click: focus  ·  Home: overview".into()};
        self.chrome.text(cx,foot.x+18.0,foot.y+10.0,9.0,p::secondary(),&f::shorten(&status,((foot.w*0.55-25.0)/6.0).max(1.0) as usize));
        let frame=self.state.frame;
        let telemetry=format!("{} visible{}  ·  CPU draw {:.2} ms  ·  Cache {}",f::count(frame.visible_nodes as u64),if frame.capped{" (LOD cap)"}else{""},frame.cpu_submit_ms,f::bytes(self.state.session.cache.bytes() as u64));
        self.chrome.text(cx,foot.x+foot.w*0.58,foot.y+10.0,9.0,p::muted(),&f::shorten(&telemetry,((foot.w*0.42-16.0)/6.0).max(1.0) as usize));
