...
impl Session{
    ...
    fn relayout(&mut self){let Some(snapshot)=&self.snapshot else{return;};self.layout_tree(snapshot.tree.clone(),None);}
    fn layout_tree(&mut self,tree:Arc<Tree>,obsolete:Option<Snapshot>){
        ...
        std::thread::spawn(move||{drop(obsolete);let _=tx.send(JobEvent::Complete(Ok((Snapshot::with_bounds(tree,metric,bounds),metric))));wake();});
    }
    pub fn poll(&mut self)->bool{
        let mut changed=self.documents.poll();
        loop{
            let event=match self.rx.as_ref().map(|rx|rx.try_recv()){
                Some(Ok(event))=>event,Some(Err(mpsc::TryRecvError::Disconnected))=>JobEvent::Complete(Err("Index worker ended without a snapshot".into())),_=>break,
            };changed=true;
            match event{
                ...
                JobEvent::Complete(result)=>{
                    ...
                    match result{
                        Ok((snapshot,metric))=>{
                            ...
                            if metric!=self.requested_metric||!same_shape{self.layout_tree(snapshot.tree.clone(),Some(snapshot));break;}
                            self.root=snapshot.tree.root.clone();let old=self.snapshot.replace(Arc::new(snapshot));if let Some(old)=old{std::thread::spawn(move||drop(old));}
                            self.metric=metric;self.selected=self.selected.or(Some(0));self.focus_pending=self.selected;self.status="Ready".into();self.update_matches();
                        }
    ...
    pub fn resize(&mut self,view:Box2){if view.w<1.0||view.h<1.0{return;}let next=layout::world_for_viewport(view);let change=(next.w-self.bounds.w).abs()>0.5;self.viewport=view;self.bounds=next;if change&&self.snapshot.is_some()&&!self.busy(){self.relayout();}}
