...
impl Default for Session {
    fn default() -> Self {
        Self { root: PathBuf::new(), options: ScanOptions::default(), snapshot: None,
            camera: Camera::default(), selected: None, hovered: None, focus_pending: Some(0),
            query: String::new(), matches: HashSet::new(), matched_files: 0, sources: true,
            status: "Open a source directory".into(), metric: AreaMetric::default(),
            requested_metric: AreaMetric::default(), rx: None,
            cancel: Arc::new(AtomicBool::new(false)), wake: noop }
    }
}
...
impl Session {
    ...
    pub fn open(&mut self, root: PathBuf, options: ScanOptions, wake: Wake) {
        ...
        self.snapshot = None; self.selected = None; self.hovered = None;
        self.matches.clear(); self.matched_files = 0;
        self.focus_pending = Some(0); self.status = format!("Indexing {}", root.display());
        let (tx, rx) = mpsc::channel(); self.rx = Some(rx);
        let cancel = self.cancel.clone(); let metric = self.requested_metric;
        std::thread::spawn(move || {
            let result = scan(&root, &options, &cancel).map(|tree| (Snapshot::new(Arc::new(tree), metric), metric));
    ...
    fn relayout(&mut self) {
        ...
        let (tx, rx) = mpsc::channel(); self.rx = Some(rx);
        self.status = "Rebuilding spatial layout".into();
    ...
    pub fn poll(&mut self) -> bool {
        let result = match self.rx.as_ref().map(|rx| rx.try_recv()) {
            ...
            Some(Err(mpsc::TryRecvError::Disconnected)) => Err("Index worker ended without a snapshot".into()),
            _ => return false,
        };
        self.rx = None;
        match result {
            Ok((snapshot, metric)) => {
                ...
                self.status = "Ready · actual source at every scale".into(); self.update_matches();
