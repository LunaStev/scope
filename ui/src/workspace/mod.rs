...
impl Default for ViewState {
    fn default() -> Self {
        Self { session: Session::default(), viewport: Box2::default(), panel: Box2::default(),
            drag_at: None, details: true, panel_scroll: 0.0, panel_height: 0.0, frame: RenderStats::default() }
    }
}
impl Workspace {
    pub fn open(&mut self, cx: &mut Cx, root: PathBuf, options: ScanOptions) {
        self.state.panel_scroll = 0.0;
        self.state.session.open(root, options, SignalToUI::set_ui_signal);
        self.redraw_view(cx);
    }
