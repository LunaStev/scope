        ...
        match event.hits(cx,self.draw_bg.area()) {
            Hit::FingerDown(e) => {
                if self.state.viewport.contains(e.abs.x,e.abs.y) {
                    ...
                    if e.tap_count>=2 {
                        if let Some(id)=self.state.session.selected { self.state.session.read_at(id,e.abs.x,e.abs.y); }
                    }
                    self.redraw_view(cx);
                }
            }
            Hit::FingerMove(e) => {
                if let Some(last)=self.state.drag_at {
                    self.state.session.camera.pan(e.abs.x-last.x,e.abs.y-last.y);
                    self.state.drag_at=Some(e.abs); self.redraw_view(cx);
                }
            }
            ...
            Hit::FingerScroll(e) => {
                if self.state.details && self.state.panel.contains(e.abs.x,e.abs.y) {
                    ...
                    self.state.session.camera.zoom_at(wheel_zoom_factor(e.scroll.y),e.abs.x,e.abs.y);
                    self.redraw_view(cx);
                }
            }
