...
impl Workspace {
    pub(super) fn input(&mut self,cx:&mut Cx,event:&Event) {
        if self.state.session.poll(){self.state.panel_dirty=true;self.redraw_view(cx);}
        if matches!(event,Event::NextFrame(_))&&self.state.frame.pending_tiles>0{self.redraw_view(cx);}
        match event.hits(cx,self.draw_bg.area()) {
            Hit::FingerDown(e)=>{
                if self.state.viewport.contains(e.abs.x,e.abs.y) {
                    cx.set_key_focus(self.draw_bg.area());
                    let selected=self.state.session.hit(e.abs.x,e.abs.y);
                    if selected!=self.state.session.selected{self.state.panel_dirty=true;self.state.panel_scroll=0.0;}
                    self.state.session.selected=selected;self.state.drag_at=Some(e.abs);
                    if e.tap_count>=2{if let Some(id)=selected{self.state.session.read_at(id,e.abs.x,e.abs.y);}}
                    self.redraw_view(cx);
                }
            }
            Hit::FingerMove(e)=>{
            ...
            Hit::FingerScroll(e)=>{
                if self.state.panel.w>0.0&&self.state.panel.contains(e.abs.x,e.abs.y) {
                    let scroll=(self.state.panel_scroll+e.scroll.y).clamp(0.0,(self.state.panel_height-self.state.panel.h).max(0.0));
                    if scroll!=self.state.panel_scroll{self.state.panel_scroll=scroll;self.state.panel_dirty=true;self.redraw_view(cx);}
            ...
            Hit::FingerHoverIn(e)|Hit::FingerHoverOver(e)=>{
                if self.state.drag_at.is_some(){return;}
            ...
            Hit::KeyDown(e)=>{
                ...
                self.state.panel_dirty=true;self.state.panel_scroll=0.0;self.redraw_view(cx);
            }
            _=>{}
        }
    }
