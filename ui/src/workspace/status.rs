//! The existing 24 px row: map preparation, optional telemetry, renderer last.
...
#[derive(Debug)]
struct FooterRegions { status: Box2, telemetry: Option<Box2>, renderer: Box2 }
impl FooterRegions {
    fn new(foot: Box2) -> Self {
        ...
        let telemetry = (width >= 900.0).then(|| Box2::new(renderer.x - gap - 260.0, foot.y, 260.0, foot.h));
        let status_right = telemetry.map_or(renderer.x, |b| b.x) - gap;
        let status = Box2::new(foot.x + padding, foot.y, (status_right - foot.x - padding).max(0.0), foot.h);
...
impl Workspace {
    pub(super) fn status_bar(&mut self, cx: &mut Cx2d, foot: Box2) {
        ...
        let status = if session.busy() && session.snapshot.is_none() {
            session.status.clone()
        } else if let Some(error) = self.map.maps.error.as_ref() {
            format!("Map: {error}")
        } else if session.snapshot.is_some() && !frame.map_ready {
            format!("Preparing complete map · {} / {} files", f::count(frame.map_completed), f::count(frame.map_total))
        } else if frame.image_pending > 0 {
            format!("Refining {} regions · overview remains visible", f::count(frame.image_pending as u64))
        ...
        if let Some(bounds) = slots.telemetry {
            let telemetry = format!("{} image tiles · {:.2} ms CPU", frame.image_tiles, frame.cpu_submit_ms);
    ...
    pub(super) fn graphics_details(&mut self, cx: &mut Cx2d, x: f64, mut y: f64, width: f64) -> f64 {
        ...
        y += 22.0;
        self.chrome.text(cx, x, y, 10.0, p::text(), "Code map"); y += 25.0;
        let map = &self.map.maps;
        for (label,value) in [
            ("Preparation", f::duration(map.prepare_ms)),
            ("Map images", f::count(self.map.images.len() as u64)),
            ("Overview cache", if map.cache_hit {"Reused from disk".into()} else {"Prepared locally".into()}),
        ] {
            self.chrome.text(cx, x, y, FONT_SIZE, p::muted(), label);
            self.chrome.text_right(cx, x + width, y, FONT_SIZE, p::secondary(), &value); y += 22.0;
        }
        y += 14.0;
        self.chrome.fill(cx, Box2::new(x, y, width, 1.0), p::border()); y += 22.0;
        self.chrome.text(cx, x, y, 10.0, p::text(), "Graphics renderer"); y += 25.0;
        let info = &self.state.graphics.info;
        for (label,value) in [("Renderer",info.renderer()),("Vendor",info.vendor())] {
            self.chrome.text(cx, x, y, FONT_SIZE, p::muted(), label); y += 17.0;
            for line in wrap_text(value, width, |s| measure(cx, &mut self.chrome, s)) {
                self.chrome.text(cx, x, y, FONT_SIZE, p::secondary(), &line); y += 17.0;
            }
            y += 8.0;
        }
        if info.software_hint() { self.chrome.text(cx, x, y, FONT_SIZE, p::muted(), "Software renderer"); y += 19.0; }
        y + 12.0
    }
}
...
fn draw_slot(cx: &mut Cx2d, paint: &mut MapPainter, bounds: Box2, text: &str, right: bool) {
    let text=fit_text(text,bounds.w,|s|measure(cx,paint,s)); if text.is_empty(){return;}
    clip(cx,bounds);
    if right {paint.text_right(cx,bounds.x+bounds.w,bounds.y+7.0,FONT_SIZE,p::muted(),&text);}
    else {paint.text(cx,bounds.x,bounds.y+7.0,FONT_SIZE,p::muted(),&text);}
    cx.end_turtle();
}
fn fit_text(text:&str,width:f64,mut measure:impl FnMut(&str)->f64)->String {
    ...
    while lo<hi {let mid=(lo+hi+1)/2;let end=ends.get(mid).copied().unwrap_or(text.len());
        if measure(&format!("{}…",&text[..end]))<=width{lo=mid;}else{hi=mid-1;}}
    format!("{}…",&text[..ends.get(lo).copied().unwrap_or(text.len())])
}
fn wrap_text(text:&str,width:f64,mut measure:impl FnMut(&str)->f64)->Vec<String> {
    ...
    for ch in text.chars(){let mut candidate=line.clone();candidate.push(ch);
        if !line.is_empty()&&measure(&candidate)>width{lines.push(std::mem::take(&mut line));}line.push(ch);}
    if !line.is_empty(){lines.push(line);}lines
}
#[cfg(test)] mod tests {
    use super::*;
    fn cells(text:&str)->f64{text.chars().count() as f64}
    #[test] fn device_and_progress_never_overlap_at_any_window_width(){
        for width in [0.0,12.0,28.0,80.0,320.0,720.0,1040.0,1480.0,3840.0]{
            let r=FooterRegions::new(Box2::new(47.0,300.0,width,24.0));
            assert!(r.renderer.w<=240.0);assert!(r.status.w>=0.0);assert!(r.status.x+r.status.w<=r.renderer.x+0.001);
            if let Some(t)=r.telemetry{assert!(r.status.x+r.status.w<=t.x);assert!(t.x+t.w<=r.renderer.x);}
            assert!(r.renderer.x+r.renderer.w<=47.0+width+0.001);assert_eq!(r.renderer.h,24.0);
        }
    }
    #[test] fn compact_window_removes_telemetry_before_device_or_progress(){let r=FooterRegions::new(Box2::new(0.0,0.0,720.0,24.0));assert!(r.telemetry.is_none());assert!(r.status.w>r.renderer.w);}
    #[test] fn long_names_are_width_fitted_and_unicode_safe(){
        assert_eq!(fit_text("llvmpipe (LLVM 20, 256 bits)",10.0,cells),"llvmpipe …");assert_eq!(fit_text("그래픽장치",3.0,cells),"그래…");
        assert_eq!(fit_text("GPU",3.0,cells),"GPU");assert_eq!(fit_text("GPU",0.0,cells),"");assert_eq!(fit_text("GPU",0.5,cells),"");
    }
    #[test] fn full_renderer_name_is_preserved_in_details(){let name="Device (driver / version / PCIe)";let lines=wrap_text(name,9.0,cells);assert_eq!(lines.concat(),name);assert!(lines.iter().all(|s|cells(s)<=9.0));}
}
