...
pub struct Scene{pub tree:Arc<Tree>,pub rectangles:Arc<[Box2]>,pub pages:Arc<[Option<SourceLayout>]>,pub fingerprint:String,pub max_file_bytes:u64}
impl Scene{
...
#[derive(Clone,Debug,Default)]pub struct Report{pub files:u64,pub lines:u64,pub errors:u64,pub error_sample:Option<String>,pub cache_hit:bool,pub source_reads:u64,pub source_hits:u64,pub glyph_rows:u64}
        ...
        for id in leaves{
            ...
            for column in &page.packed{
                ...
                for line in first..end{
                    ...
                    lexer.spans(value,|a,b,ink|{let color=ink_color(ink);for ch in value[a..b].chars(){if ch=='\t'{cell+=4-cell%4;continue;}let x=cx+cell as f64*page.font_size*layout::source::GLYPH_ADVANCE*sx;if x+page.font_size*sx>=0.0&&x<size as f64{self.glyphs.paint(&mut image,ch,x,y,page.font_size*sx,page.font_size*sy,color);}cell+=1;}});
