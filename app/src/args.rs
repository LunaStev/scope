...
pub fn parse(args:impl IntoIterator<Item=OsString>)->Result<Option<Arguments>,String>{
    ...
    while let Some(arg)=args.next(){match arg.to_str(){
        Some("--threads")=>{let value=args.next().ok_or("--threads needs a number between 1 and 32")?;let n:usize=value.to_str().ok_or("invalid worker count")?.parse().map_err(|_|"invalid worker count")?;if !(1..=32).contains(&n){return Err("--threads must be between 1 and 32".into());}out.options.threads=n;}
        ...
        Some("--max-file-mib")=>{let value=args.next().ok_or("--max-file-mib needs a positive integer")?;let value:u64=value.to_str().ok_or("invalid size")?.parse().map_err(|_|"invalid size")?;if !(1..=1024).contains(&value){return Err("--max-file-mib must be between 1 and 1024".into());}out.options.max_file_bytes=value*1024*1024;}
        Some("--help"|"-h")=>return Ok(None),
        Some("--")=>{out.root=args.next().ok_or("expected a path after --")?.into();paths+=1;if args.next().is_some(){return Err("only one root path is accepted".into());}}
...
#[cfg(test)]mod tests{
    use super::*;fn argv(s:&[&str])->Vec<OsString>{s.iter().map(OsString::from).collect()}
    ...
    #[test]fn validates_worker_limits(){for n in ["0","33","-1","abc"]{assert!(parse(argv(&["--threads",n])).is_err());}assert_eq!(parse(argv(&["--threads","4"])).unwrap().unwrap().options.threads,4);}
