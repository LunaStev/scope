use model::{AreaMetric,Document,FileInfo,Node,Stats,Tree};
use layout::{tree_layout,hit_test};
use std::sync::Arc;
fn tree()->Tree{
    let mut t=Tree::new("fixture".into());
    for(name,lines)in[("a.rs",100),("b.rs",10),("empty.rs",0)]{
        let id=t.nodes.len();t.nodes.push(Node{name:name.into(),relative:name.into(),parent:Some(0),children:vec![],stats:Stats{files:1,lines,code:lines,bytes:lines*10,..Stats::default()},file:Some(FileInfo{language:"Rust".into(),classified:true,document:Arc::new(Document::new("x\n".repeat(lines as usize)))})});
