...
impl Camera {
    ...
    pub fn zoom_at(&mut self,factor:f64,x:f64,y:f64){
        ...
        let(wx,wy)=self.unproject(x,y);self.scale=(self.scale*factor).clamp(0.00001,1_000_000.0);
    ...
    pub fn read_page(&mut self,page:&SourceLayout,viewport:Box2){
        self.fit(page.content,viewport);self.scale=self.scale.max(READING_SIZE/page.font_size).clamp(0.00001,1_000_000.0);
        self.x=viewport.x+24.0-page.content.x*self.scale;self.y=viewport.y+24.0-page.content.y*self.scale;
    }
...
mod tests{
    use super::*;
    #[test]fn cursor_anchor_survives_zoom(){let mut c=Camera::default();let before=c.unproject(120.0,50.0);c.zoom_at(4.0,120.0,50.0);assert_eq!(before,c.unproject(120.0,50.0));}
    #[test]fn fit_is_finite_for_empty_view(){let mut c=Camera::default();c.fit(Box2::default(),Box2::default());assert!(c.scale.is_finite());c.zoom_at(f64::NAN,0.0,0.0);assert!(c.scale.is_finite());}
    #[test]fn wheel_is_faster_and_reversible(){assert!(wheel_zoom_factor(-10.0)>1.2);assert!((wheel_zoom_factor(10.0)*wheel_zoom_factor(-10.0)-1.0).abs()<1e-12);}
    #[test]fn double_click_reaches_readable_text_without_reflow(){
        let page=SourceLayout::new(Box2::new(0.0,0.0,400.0,200.0),1000,80);
        let mut c=Camera::default();let anchor=c.unproject(20.0,30.0);c.read_at(&page,20.0,30.0);
        assert!((c.scale*page.font_size-READING_SIZE).abs()<1e-10);
        let after=c.unproject(20.0,30.0);assert!((anchor.0-after.0).abs()<1e-10);
    }
}
