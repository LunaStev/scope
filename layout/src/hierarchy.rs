...
pub fn tree_layout(tree:&Tree,metric:AreaMetric)->Vec<Box2> {
    ...
    while let Some(id)=stack.pop(){
        ...
        let p=(bounds.w.min(bounds.h)*0.006).min(5.0);
        let mut inner=bounds.inset(p);let header=(inner.h*0.04).min(19.0);inner.y+=header;inner.h-=header;
        let values:Vec<f64>=children.iter().map(|id|weights[*id]).collect();
        for(child,r)in children.into_iter().zip(squarify(&values,inner)){
            rects[child]=r.inset((r.w.min(r.h)*0.004).min(0.9));stack.push(child);
        }
    }
    rects
}
